    ...
    for (let x = 0; x < 32; x++) {
        for (let y = 0; y < 32; y++) {
            if ((x == 0 || x == 31) || (y == 0 || y == 31)) {
                let isShort = (x % 4 == 0 && (y == 0 || y == 31)) || (y % 4 == 0 && (x == 0 || x == 31));
                let scaleY = isShort ? 0.25 : 0.73;
                let color = isShort ? [.80, .70, .50, 1.0] : [.60, .40, .20, 1.0];

                let wall = new Cube();
                wall.color = color;
                wall.textureNum = -2;
                wall.matrix.scale(0.25, scaleY, 0.25);
...
function drawWall(x, y, scaleY, color) {
    var wall = new Cube();
    wall.color = color;
    wall.textureNum = -2;
    wall.matrix.scale(0.25, scaleY, 0.25);
...
function drawMap() {
    // Draw the fence (static walls)
    for (let x = 0; x < 32; x++) {
        for (let y = 0; y < 32; y++) {
            if ((x == 0 || x == 31) && y % 4 == 0) {
                drawWall(x, y, 0.25, [.80, .70, .40, 1.0]); // Short walls
            }
            else if ((x == 0 || x == 31)) {
                drawWall(x, y, 0.73, [.60, .40, .20, 1.0]); // Tall walls
            }
            else if ((y == 0 || y == 31) && x % 4 == 0) {
                drawWall(x, y, 0.25, [.80, .70, .50, 1.0]); // Short walls
            }
            else if ((y == 0 || y == 31)) {
                drawWall(x, y, 0.73, [.60, .40, .20, 1.0]); // Tall walls
            }
    ...
    // Draw dynamically placed blocks
